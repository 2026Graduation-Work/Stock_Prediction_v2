...
| 2 | #29 → #30 → #31 | 온보딩 → 인증 대시보드 → 실험 artifact | 앞 PR 병합 후 차례로 `main` retarget |
| 3 | #27 → #28 | 평가 경계 보완 → 비교 러너 실행 준비 | CI 확인 후 순차 병합 |
| 보류 | #22 | 가치투자 오케스트레이션 | #25 경로 계약 반영, 충돌 해소, fallback 검증 필요 |
| 보류 | #26 | 외부 피처 결합 | 누수·타깃 혼입·pandas 호환 리뷰 해결 필요 |

병합 원칙:

- 같은 파일을 만지는 체인은 반드시 앞 PR부터 병합하고 다음 PR의 base를 `main`으로 바꾼다.
- #22와 #26은 열린 high finding을 해결하기 전 승인하지 않는다.
