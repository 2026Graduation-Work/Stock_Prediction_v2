...
> ⚠️ 2026-09-22 기준 일부 내용이 현재 구현과 다르다. 현재 상태는 [README](../README.md)와 [문서 색인](README.md)을 본다.
...
| 5 | (#36, #38) → #22 | BigKinds 경로·LLM HITL 경계 → 가치투자 오케스트레이션 | 두 보조 PR 병합 후 #22의 main 충돌 해소·전체 test |
...
- #22는 보조 PR #36·#38을, #26은 #37을 먼저 병합하고 부모 브랜치에서 CI와 리뷰를
  다시 확인하기 전 승인하지 않는다.
...
- #38을 #22에 병합해 LLM `reasoning`을 수치·이벤트 설명으로 한정하고, 행동 제안은
  결정론적 필터와 수치 템플릿으로 대체
