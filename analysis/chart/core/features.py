...
def calculate_dynamic_triple_barrier(df: pd.DataFrame, horizon=5, up_mult=1.5, down_mult=1.2) -> pd.DataFrame:
    """
    ...
    trading_log_ret = df['Log_Ret'].where(
        df.get('Trading_Halt', pd.Series(0, index=df.index)) == 0
    )
