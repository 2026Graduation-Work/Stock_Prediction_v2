...
import pandas as pd
from .features import generate_full_alpha158_features, normalize_trading_halts, calculate_dynamic_triple_barrier
...
FEATURE_COLS = [
    ...
    'open_0', 'high_0', 'low_0', 'vwap_0'
]
# W일 Window 기반 피처 동적 추가 (5, 10, 20, 30, 60)
for w in [5, 10, 20, 30, 60]:
    FEATURE_COLS.extend([
        f'roc_{w}', f'ma_{w}', f'max_{w}', f'min_{w}', f'rsv_{w}', f'std_{w}',
        f'beta_{w}', f'rsqr_{w}', f'resi_{w}', f'rank_{w}', f'qtlu_{w}', f'qtld_{w}',
        f'imax_{w}', f'imin_{w}', f'imxd_{w}', f'cntp_{w}', f'cntn_{w}', f'cntd_{w}',
        f'sump_{w}', f'sumn_{w}', f'sumd_{w}', f'corr_{w}', f'cord_{w}',
        f'vma_{w}', f'vstd_{w}', f'wvma_{w}', f'vsump_{w}', f'vsumn_{w}', f'vsumd_{w}'
    ])
