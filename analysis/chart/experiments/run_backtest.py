...
def main(config_path):
    ...
    # 캐시된 예측 확률 확인
    current_dir = os.path.dirname(os.path.abspath(__file__))
    predictions_cache_path = os.path.join(current_dir, "cache", f"{predictions_hash}_predictions.parquet")
    ...
    if not os.path.exists(predictions_cache_path):
        ...
        print(f"   -> 백테스트를 돌리려면 먼저 모델 학습 및 OOS 예측 캐시를 생성해야 합니다.")
        print(f"   -> 해결방법: 먼저 아래 명령어를 수행하여 모델 예측 결과를 빌드해 주세요.")
        print(f"      python experiments/train.py --config {config_path}")
        return
        
    print(f"⚡ [CACHE HIT] 캐시된 OOS 예측 결과를 로드합니다.")
    ...
    market_df = load_parquet_data(processed_dir, full_test_start, full_test_end, columns_only=price_cols, tickers=tickers_cfg)
