...
def apply_fixed_barrier_labeling(df: pd.DataFrame, horizon: int, tp_pct: float, sl_pct: float) -> pd.Series:
...
def apply_dynamic_sigma_barrier_labeling(df: pd.DataFrame, horizon: int, up_mult: float, down_mult: float) -> pd.Series:
...
def load_parquet_data(data_dir: str, start_date: str = None, end_date: str = None, columns_only: list = None, tickers: str = None, label_params: dict = None, training: bool = False, keep_date: bool = False) -> pd.DataFrame:
    """
        ...
        try:
            ...
            if label_params is not None:
                ...
                if label_type == 'dynamic_sigma':
                    ...
                    y_label_series = apply_dynamic_sigma_barrier_labeling(temp_df, horizon, up_mult, down_mult)
                else:
            ...
            if training:
                exclude_cols = ['Code', 'Name', 'Open', 'High', 'Low', 'Close', 'Volume', 'IsDelisted', 'Log_Ret', 'Sigma', 'Y_Label', 'Trading_Halt']
                if not keep_date:
