...
def parse_args():
    parser = argparse.ArgumentParser(description="전체 종목 대상 병렬 추론 및 매수 시그널 생성 스크립트 (단일 모델)")
    parser.add_argument(
        "--model-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="매수 시그널 최소 확률 임계값 (0.0 ~ 1.0)"
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=30,
        help="터미널에 출력할 상위 종목 수"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def main():
    ...
    if not results:
        print("❌ [Error] 유효한 추론 결과가 없습니다. 데이터 수집 상태나 --target-date를 확인하세요.")
        return
    ...
    # 5. 상위 종목 출력
    print("\n" + "="*85)
    print(f"📊 상승 예측 확률 상위 {args.top_n}개 종목 (기준 영업일: {unique_dates[0] if len(unique_dates) == 1 else '복수'})")
    print("="*85)
    ...
    print(f"💡 총 {len(result_df)}개 종목 중 'BUY' 시그널 발생: {len(buy_signals)}개 (임계치: {args.threshold*100:.1f}%)")
    print("="*85)
