...
def find_processed_dir() -> str:
    ...
    candidate_paths = [
        os.path.abspath(os.path.join(current_dir, "..", "data", "processed")),  # 블록 내부 data/processed
        os.path.abspath(os.path.join(current_dir, "..", "..", "..", "data", "processed")),  # 프로젝트 루트 data/processed
...
def main(config_path):
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"[ERROR] 설정을 불러올 수 없습니다. 경로를 확인해주세요: {config_path}")
    ...
    print(f"[*] 데이터 분할 전략: {config.get('data', {}).get('split_strategy', 'single')} (총 {len(splits)}개 폴드)")
    for idx, split_info in enumerate(splits):
        print(f"    - [Fold {idx+1}] Train: {split_info['train_start']} ~ {split_info['train_end']} | Test: {split_info['test_start']} ~ {split_info['test_end']}")
    ...
    if not os.path.exists(processed_dir) or not os.listdir(processed_dir):
        print(f"[!] 경고: {processed_dir}가 비어있거나 존재하지 않습니다. 먼저 수집 및 전처리를 실행해야 할 수 있습니다.")
    ...
    if os.path.exists(predictions_cache_path):
        ...
        for idx, split_info in enumerate(splits):
            ...
            fe_save_path = os.path.join(models_dir, f"{fold_pred_hash}_fe.pkl")
            ...
            if os.path.exists(model_save_path):
                ...
                val_start_dt = pure_train_end_dt + pd.Timedelta(days=1)
                ...
                train_df = load_parquet_data(processed_dir, train_start, pure_train_end, tickers=tickers_cfg, label_params=label_params, training=True, keep_date=False)
                ...
                train_bin_path = os.path.join(model_wrapper.cache_dir, f"{fold_data_hash}_train.bin")
                ...
                val_df = load_parquet_data(processed_dir, val_start, train_end, tickers=tickers_cfg, label_params=label_params, training=False)
                ...
                if len(daily_ic) < 5:
                    raise ValueError(f"[-] Validation 기간의 유효 거래일 수({len(daily_ic)})가 너무 적어 통계 검정이 불가능합니다.")
                ...
                if n_signals == 0:
                    ...
                    p_val_binom = 1.0 - stats.binom.cdf(n_success - 1, n_signals, base_rate) if n_success > 0 else 1.0
                ...
                print("=" * 85)
                print(f"  - Mean Rank IC:   {mean_ic:.4f}  (Threshold: >= 0.02) -> {'PASSED ✅' if ic_passed else 'FAILED ❌'}")
                print(f"  - t-statistic:    {t_stat:.4f}  (Threshold: >= 2.0)  -> {'PASSED ✅' if t_stat >= 2.0 else 'FAILED ❌'}")
                print(f"  - Success Rate:   {success_rate*100:.2f}% (Base Rate: {base_rate*100:.2f}%)")
                print(f"  - Binomial p-val: {p_val_binom:.6f}  (Threshold: <= 0.05) -> {'PASSED ✅' if binom_passed else 'FAILED ❌'}")
                print("=" * 85)
            ...
            test_df = load_parquet_data(processed_dir, test_start, test_end, columns_only=feature_cols + ['Date', 'Code'], tickers=tickers_cfg)
