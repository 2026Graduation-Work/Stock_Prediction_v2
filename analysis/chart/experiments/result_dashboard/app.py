...
def resolve_splits(config: dict) -> list:
    ...
    if strategy == 'single':
        ...
        if 'custom_blocks' in data_cfg:
            ...
            return [
                {"train_start": "2016-01-01", "train_end": "2018-12-31", "test_start": "2019-01-08", "test_end": "2019-12-31"},
                {"train_start": "2018-01-01", "train_end": "2020-12-31", "test_start": "2021-01-08", "test_end": "2021-12-31"},
                {"train_start": "2021-01-01", "train_end": "2023-12-31", "test_start": "2024-01-08", "test_end": "2025-12-31"}
            ]
        ...
        for y in range(sy + tw, ey - te + 1):
            ts = (pd.to_datetime(f"{y - 1}-12-31") + pd.Timedelta(days=embargo_days)).strftime("%Y-%m-%d")
            folds_info.append({
...
def main():
    ...
    st.markdown("백테스트 결과를 한눈에 분석하고, 거시경제 이벤트와 결합하여 리버스 엔지니어링을 수행합니다.")
    ...
    view_mode = st.sidebar.radio("📊 대시보드 뷰 선택:", ["📈 기본 성과 대시보드", "📝 QuantStats 상세 분석 리포트"])
    ...
    if view_mode == "📝 QuantStats 상세 분석 리포트":
        ...
        if not ew_exists and not krx_exists:
            st.warning("⚠️ QuantStats 보고서 파일이 없습니다. 백테스트를 실행하면 자동으로 생성됩니다.")
            return
        ...
        for tab, path, label in zip(tabs, tab_paths, tab_labels):
            with tab:
                if "EW" in label:
                    st.info("📌 **벤치마크 해석**: 전략이 우리 유니버스 전 종목을 동일 비중으로 단순 보유했을 때 대비 **얼마나 더 나은 종목 선택 능력**을 보였는지 측정합니다. (학술적 알파 측정 기준)")
                else:
                    st.info("📌 **벤치마크 해석**: 전략이 코스피·코스닥 시장 전체 흐름(KOSPI+KOSDAQ 시가총액 가중 합산) 대비 **얼마나 더 나은 시장 초과 수익**을 냈는지 측정합니다. (직관적 시장 기준)")
    ...
    if config:
        st.write("")  # 간격 띄우기
        with st.expander("⚙️ 모델 하이퍼파라미터 & 백테스트 기준 정보 (Active Configuration)", expanded=False):
            ...
            with col_cfg1:
                ...
                if config.get('data', {}).get('split_strategy') == 'sliding':
                    st.write(f"  - `Train Window`: {config.get('data', {}).get('sliding', {}).get('train_window_years', '-')}년")
                    st.write(f"  - `Test Window`: {config.get('data', {}).get('sliding', {}).get('test_window_years', '-')}년")
            
            with col_cfg2:
                st.markdown("#### 🤖 LightGBM 트리 모델 설정")
                model_params = config.get('model', {}).get('params', {})
                st.write(f"- **트리 개수 (n_estimators)**: `{model_params.get('n_estimators', '-')}`")
                st.write(f"- **학습률 (learning_rate)**: `{model_params.get('learning_rate', '-')}`")
                st.write(f"- **최대 트리 깊이 (max_depth)**: `{model_params.get('max_depth', '-')}`")
                st.write(f"- **리프 노드 수 (num_leaves)**: `{model_params.get('num_leaves', '-')}`")
                st.write(f"- **최소 샘플 규제 (min_child_samples)**: `{model_params.get('min_child_samples', '-')}`")
                
            with col_cfg3:
                st.markdown("#### 🛡️ 트레이딩 및 백테스트 기준")
                st.write(f"- **최소 확률 컷 (prob_threshold)**: `{config.get('strategy', {}).get('prob_threshold', '-')}`")
                st.write(f"- **일일 선택 종목 수 (top_n)**: `{config.get('strategy', {}).get('top_n', '-')}개`")
                st.write(f"- **익절 배리어 승수 (up_mult)**: `{config.get('backtest', {}).get('up_mult', '-')}σ`")
                st.write(f"- **손절 배리어 승수 (down_mult)**: `{config.get('backtest', {}).get('down_mult', '-')}σ`")
                st.write(f"- **거래 비용 (수수료+세금)**: `{config.get('backtest', {}).get('fee', 0.0025)*100:.3f}%` (편도)")
    ...
    if os.path.exists(daily_returns_path):
        ...
        if 'Benchmark_CustomKRX' in ret_df.columns:
            plot_df['🇰🇷 커스텀 KRX 통합 지수 (시장 기준)'] = (1 + ret_df['Benchmark_CustomKRX']).cumprod()
        ...
        if 'Benchmark_CustomKRX' in ret_df.columns:
            with st.expander("📐 커스텀 KRX 통합 지수 산출 방법론 (클릭하여 펼치기)", expanded=False):
                st.markdown("""
...
""")
    ...
    if os.path.exists(trades_path):
        ...
        if len(trades_df) > 0:
            ...
            metadata_candidates = [
                os.path.abspath(os.path.join(current_dir, "..", "..", "data", "ticker_metadata.csv")), # analysis/chart/data
                os.path.abspath(os.path.join(current_dir, "..", "..", "..", "data", "ticker_metadata.csv")), # Stock_Prediction_v2/data
            ...
            def extract_code(col_str):
                ...
                try:
                    ...
                    return col_str.replace("'", "").replace("\"", "").replace("(", "").replace(")", "").split(",")[0].strip()
            ...
            display_cols = []
            target_cols = [
                'Entry Timestamp', 'Exit Timestamp', 'Column', 'Direction', 'Size', 
                'Entry Price', 'Avg Entry Price', 'Exit Price', 'Avg Exit Price', 
                'Return', 'PnL'
            ]
            ...
            if display_cols:
                ...
                # 2. 총 투자금액(원) 계산
                entry_price_col = 'Avg Entry Price' if 'Avg Entry Price' in filtered_df.columns else ('Entry Price' if 'Entry Price' in filtered_df.columns else None)
                if entry_price_col and 'Size' in filtered_df.columns:
                    filtered_df['총 투자금액(원)'] = (filtered_df[entry_price_col] * filtered_df['Size']).round(0).astype(int)

                # Direction 한글 변환
                if 'Direction' in filtered_df.columns:
                    filtered_df['Direction'] = filtered_df['Direction'].map({'Long': '매수 (Long)', 'Short': '매도 (Short)'}).fillna(filtered_df['Direction'])
                ...
                # 출력 순서 정렬 (시장 컬럼 추가)
                column_order = [
                    '진입일시', '청산일시', '종목', '시장', '포지션', '거래수량(주)', 
                    '주당 진입가격(원)', '주당 청산가격(원)', '총 투자금액(원)', 
                    '수익률(%)', '실현손익(원)', '청산 구분'
                ]
                ...
                st.dataframe(
                    ...
                    height=350,
                    column_config={
                        "진입일시": st.column_config.DatetimeColumn("진입일시", format="YYYY-MM-DD"),
                        "청산일시": st.column_config.DatetimeColumn("청산일시", format="YYYY-MM-DD"),
                        "종목": st.column_config.TextColumn("종목"),
                        "시장": st.column_config.TextColumn("시장"),
                        "포지션": st.column_config.TextColumn("포지션"),
                        "거래수량(주)": st.column_config.NumberColumn("거래수량(주)", format="%.2f"),
                        "주당 진입가격(원)": st.column_config.NumberColumn("주당 진입가격(원)", format="₩%,d"),
                        "주당 청산가격(원)": st.column_config.NumberColumn("주당 청산가격(원)", format="₩%,d"),
                        "총 투자금액(원)": st.column_config.NumberColumn("총 투자금액(원)", format="₩%,d"),
                        "수익률(%)": st.column_config.NumberColumn("수익률(%)", format="%.2f%%"),
                        "실현손익(원)": st.column_config.NumberColumn("실현손익(원)", format="₩%,d"),
        ...
        try:
            ...
            if model_files:
                ...
                fi_df = pd.DataFrame({
                    ...
                    'Importance (Gain)': avg_importances
                })
                ...
                # 정렬 및 필터링
                fi_df = fi_df.sort_values('Importance (Gain)', ascending=False).reset_index(drop=True)
                ...
                top_n_display = st.slider("표시할 상위 피처 개수를 선택하세요:", min_value=5, max_value=50, value=20, step=5)
                ...
                fig_fi = px.bar(
                    ...
                    labels={'Importance (Gain)': 'Information Gain (정밀도/손실감소 기여도)', 'Feature': '피처명'},
                    color='Importance (Gain)',
                    color_continuous_scale=[[0, 'rgba(0, 184, 148, 0.3)'], [1, 'rgba(0, 184, 148, 1.0)']], # 에메랄드 톤
                ...
                with col_fi1:
                    ...
                    st.dataframe(
                        fi_df.head(10)[['Feature', 'Importance (Gain)', 'Contribution (%)']].style.format({
                            'Importance (Gain)': '{:,.1f}',
                            'Contribution (%)': '{:.2f}%'
                        }),
                        use_container_width=True
                ...
                selected_fold_idx = st.selectbox(
                    ...
                    format_func=lambda idx: f"Fold {idx+1} 모델 ({os.path.basename(model_files[idx])})"
                )
                ...
                tab1, tab2 = st.tabs(["📊 피처별 분기 임계점(Threshold) 분포 분석", "🌳 개별 의사결정 나무(Tree) 구조 분석"])
                
                with tab1:
                    st.markdown("##### 💡 피처별 의사결정 임계점(Threshold) 추출기")
                    st.markdown("특정 피처를 선택하면, 전체 의사결정나무 숲(Forest) 내에서 해당 피처가 **실제 어떤 수치들을 기준으로 분기되었는지** 모든 임계점을 추출하여 분포를 분석합니다.")
                    ...
                    def extract_thresholds(node, target_feat, feat_names):
                        ...
                        if "left_child" in node:
                            thrs.extend(extract_thresholds(node["left_child"], target_feat, feat_names))
                        if "right_child" in node:
                            thrs.extend(extract_thresholds(node["right_child"], target_feat, feat_names))
                        return thrs
                    
                    all_thresholds = []
                    for tree in model_dict.get('tree_info', []):
                        all_thresholds.extend(extract_thresholds(tree.get('tree_structure', {}), selected_feature, feature_names))
                        
                    if all_thresholds:
                        st.success(f"선택한 피처 `{selected_feature}`는 전체 {len(model_dict.get('tree_info', []))}개의 나무 중에서 **총 {len(all_thresholds)}번** 분기 기준으로 사용되었습니다.")
                        ...
                        st.markdown(f"💡 **차트 해석 가이드**: 그래프에서 빈도가 가장 높은 구간(피크 지점)은 AI 모델이 이 종목의 상태(익절/손절)를 가르는 데 **가장 중요하게 생각하는 최적의 기준선**입니다. 이 임계값 전후로 전략의 매수 신호 강도가 어떻게 변할지 정성적으로 유추할 수 있습니다.")
                    else:
                        st.warning(f"선택한 피처 `{selected_feature}`는 현재 Fold 모델의 트리 분기점에서 단 한 번도 사용되지 않았습니다.")
                        
                with tab2:
                    st.markdown("##### 🌳 개별 의사결정 나무(Tree) 구조 직접 뜯어보기")
                    st.markdown("LGBM 앙상블을 구성하는 개별 나무들의 구조와 판단 규칙을 텍스트 기반 계층형 구조로 투명하게 출력합니다.")
