...
```
yellow/      🟡 사용자 내부 정보 — 심리 설문, 사용자 프로파일링
green_cv/    🟢 차트 패턴 분석 (기술적 분석, DT 계열 모델)
green_llm/   🟢 뉴스 감성·재무제표·SNS 심리 분석 (LLM)
blue/        🔵 기능 명세, API, 대시보드, 인프라
...
- **블록(디렉터리) 경계**: 작업은 담당 블록 디렉터리(`yellow/`, `green_cv/`, `green_llm/`, `blue/`) 안에서만 한다. 여러 블록을 동시에 건드리는 변경은 PR을 분리하는 것을 우선 고려한다.
...
- **커밋 메시지**: 한국어로, `타입: 설명` 형식. 타입은 `feat`, `fix`, `docs`, `chore`, `refactor`, `test` 중 하나.
  - 예) `feat: 설문 응답 변환기 추가`
- **빌드/실행 확인**: 푸시 전 반드시 로컬에서 동작을 확인한다. 웹(`blue`)은 `npm run build`, Python 모델은 최소한 스크립트/노트북이 에러 없이 실행되는지 확인한다.
